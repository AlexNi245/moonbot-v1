...
import "./libraries/OZ/access/Ownable.sol";
import "./libraries/OZ/utils/math/SafeMath.sol";
import "./libraries/OZ/utils/structs/EnumerableSet.sol";
...
contract OffchainOracle is Ownable {
    ...
    constructor(
        ...
        unchecked {
            require(
                existingOracles.length == oracleTypes.length,
            ...
            for (uint256 i = 0; i < existingOracles.length; i++) {
                if (oracleTypes[i] == OracleType.WETH) {
                    require(
                        _wethOracles.add(address(existingOracles[i])),
                        "Oracle already added"
                    );
                } else if (oracleTypes[i] == OracleType.ETH) {
                    require(
                        _ethOracles.add(address(existingOracles[i])),
                        "Oracle already added"
                    );
            ...
            for (uint256 i = 0; i < existingConnectors.length; i++) {
                require(
                    _connectors.add(address(existingConnectors[i])),
    ...
    function oracles()
        public
        view
        returns (IOracle[] memory allOracles, OracleType[] memory oracleTypes)
    {
        unchecked {
            IOracle[] memory oraclesBuffer = new IOracle[](
                _wethOracles._inner._values.length +
                    _ethOracles._inner._values.length
            );
            OracleType[] memory oracleTypesBuffer = new OracleType[](
                oraclesBuffer.length
            );
            for (uint256 i = 0; i < _wethOracles._inner._values.length; i++) {
                oraclesBuffer[i] = IOracle(
                    address(uint160(uint256(_wethOracles._inner._values[i])))
                );
            ...
            for (uint256 i = 0; i < _ethOracles._inner._values.length; i++) {
                ...
                IOracle oracle = IOracle(
                    address(uint160(uint256(_ethOracles._inner._values[i])))
                );
    ...
    function connectors()
        external
        view
        returns (IERC20[] memory allConnectors)
    {
        unchecked {
            allConnectors = new IERC20[](_connectors.length());
            for (uint256 i = 0; i < allConnectors.length; i++) {
                allConnectors[i] = IERC20(
                    address(uint160(uint256(_connectors._inner._values[i])))
                );
            }
        }
    }
    ...
    function addOracle(IOracle oracle, OracleType oracleKind)
        external
        onlyOwner
    {
    ...
    function removeOracle(IOracle oracle, OracleType oracleKind)
        external
        onlyOwner
    {
        if (oracleKind == OracleType.WETH) {
    ...
    */
    function getRate(
        ...
        (
            IERC20[] memory wrappedSrcTokens,
            uint256[] memory srcRates
        ) = _getWrappedTokens(srcToken, useWrappers);
        (
            IERC20[] memory wrappedDstTokens,
            uint256[] memory dstRates
        ) = _getWrappedTokens(dstToken, useWrappers);
            ...
            for (uint256 k1 = 0; k1 < wrappedSrcTokens.length; k1++) {
                for (uint256 k2 = 0; k2 < wrappedDstTokens.length; k2++) {
                    ...
                    for (uint256 j = 0; j < connectors_.length; j++) {
                        if (
                            IERC20(address(uint160(uint256(connectors_[j])))) ==
                            wrappedSrcTokens[k1] ||
                            IERC20(address(uint160(uint256(connectors_[j])))) ==
                            wrappedDstTokens[k2]
                        ) {
                        ...
                        for (uint256 i = 0; i < allOracles.length; i++) {
                            try
                                allOracles[i].getRate(
                                    wrappedSrcTokens[k1],
                                    wrappedDstTokens[k2],
                                    IERC20(
                                        address(
                                            uint160(uint256(connectors_[j]))
                                        )
                                    )
                                )
        ...
        returns (uint256 weightedRate)
    {
        ...
        (
            IERC20[] memory wrappedSrcTokens,
            uint256[] memory srcRates
        ) = _getWrappedTokens(srcToken, useSrcWrappers);
        IERC20[2] memory wrappedDstTokens = [_BASE, _wBase];
        bytes32[][2] memory wrappedOracles = [
            _ethOracles._inner._values,
            _wethOracles._inner._values
        ];
        bytes32[] memory connectors_ = _connectors._inner._values;
        ...
        unchecked {
            ...
            for (uint256 k1 = 0; k1 < wrappedSrcTokens.length; k1++) {
                ...
                for (uint256 k2 = 0; k2 < wrappedDstTokens.length; k2++) {
                    ...
                    console.log("conn length : ",connectors_.length);
                    for (uint256 j = 0; j < connectors_.length; j++) {
                        IERC20 connector = IERC20(
                            address(uint160(uint256(connectors_[j])))
                        );
                        if (
                            connector == wrappedSrcTokens[k1] ||
                            connector == wrappedDstTokens[k2]
                        ) {
                            continue;
                        }
                        for (
                            uint256 i = 0;
                            i < wrappedOracles[k2].length;
                            i++
                        ) {
                                console.log("rate");
                            try
                                IOracle(
                                    address(
                                        uint160(uint256(wrappedOracles[k2][i]))
                                    )
                                ).getRate(
                                        wrappedSrcTokens[k1],
                                        wrappedDstTokens[k2],
