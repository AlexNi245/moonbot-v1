...
const MOONBEAM_PROVIDER = new ethers.providers.StaticJsonRpcProvider(
...
const SIGNER = new Wallet(process.env.PRIVATE_KEY!, MOONBEAM_PROVIDER);
...
const evaluateTransaction = async (data: string, blockNumber: number) => {
    ...
    if (affectedPools.length > 0) {
        const allOpportunities = await evaluteProfitInPools(
            MOONBEAM_PROVIDER,
            QUERY_ADDRESS,
            affectedPools,
            TARGET
        ...
        tryExecution(MOONBEAM_PROVIDER, SIGNER, EXECUTOR_ADDRESS, blockNumber, [
            trade,
        ]);
...
const getAllAllAffectedPairs = async (tokenAddresses: string[]) => {
    ...
    return Array.from(new Set(allpools)).filter(
