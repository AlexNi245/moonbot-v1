...
describe("Executor test", () => {
    ...
    beforeEach(async function () {
        ...
        const wethDaiPairOneAddress = await uniswapFactoryOne.getPair(
            weth.address,
            dai.address
        );
        const wethDaiPairTwoAddress = await uniswapFactoryTwo.getPair(
            weth.address,
            usdt.address
        );
        const wethDaiPairThreeAddress = await uniswapFactoryTwo.getPair(
            weth.address,
            dai.address
        );
    ...
    it("_calcTokenForEthh works properly if token 0 == WETH", async () => {
        ...
        const [actualAmount0Out, actualAmount1Out] =
            await executor._calcTokenForEth(buyFromPair.address, amountIn);

        const expectedAmount0 = BigNumber.from(0);
        const expectedAmount1 = BigNumber.from(996999);

        assert(
            expectedAmount0.eq(actualAmount0Out),
            `Expect amount0ToBe ${expectedAmount0} is ${actualAmount0Out}`
        );
        assert(
            expectedAmount1.eq(actualAmount1Out),
            `Expect amount1ToBe ${expectedAmount1} is ${actualAmount1Out}`
        );
    });
    ...
    it("_calcTokenForEth works properly if token 1 == WETH", async () => {
        ...
        assert(
            expectedAmount1.eq(actualAmount1Out),
            `Expect amount0ToBe ${expectedAmount1} is ${actualAmount1Out}`
        );
    });
    ...
    it("_calcEthForTokens properly if token 0 == WETH", async () => {
        ...
        const [actualAmount0Out, actualAmount1Out] =
            await executor._calcEthForTokens(sellToPair.address, amountIn);
    ...
    it("swap happy path", async () => {
        ...
        const [buyFromPairReserve0, buyFromPairReserve1] =
            await buyFromPair.getReserves();
        const [sellToPairPairReserve0, sellToPairReserve1] =
            await sellToPair.getReserves();

        const [amountIn, dir] = calcProfitMaximizingTrade(
            sellToPairPairReserve0,
            sellToPairReserve1,
            buyFromPairReserve0,
            buyFromPairReserve1
        );
        ...
        const t = await executor.populateTransaction.trade(
            amountIn,
            [buyFromPair.address, sellToPair.address],
            1
        );
